...
router.get('/me', auth, async (req, res) => {
  ...
  if (!user) {
    return res
      .status(404)
      .send(`Failed to get the user (Id: ${req.user._id}) from the database.`);
  }
...
router.post('/', async (req, res) => {
  ...
  if (existsUser) {
    return res
      .status(400)
      .send(
        `User with the email ${req.body.email} already exists on the database.`
      );
  }
  ...
  // After successful creation of the user, create a token for the user.
