...
router.post('/', [auth, validate(validateRequestRental)], async (req, res) => {
  ...
  if (!customer) {
    return res
      .status(404)
      .send(
        `Customer not found (Id: ${req.body.customerId.trim()}) on the database.`
      );
  }
  ...
  if (!movie) {
    return res
      .status(404)
      .send(
        `Movie not found (Id: ${req.body.movieId.trim()}) on the database.`
      );
  }
  ...
  if (!rental) {
    return res
      .status(404)
      .send(
        `Rental not found (customerId: ${req.body.customerId.trim()}, movieId: ${req.body.movieId.trim()}) on the database.`
      );
  }
