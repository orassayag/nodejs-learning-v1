...
const validateMovie = (movie) => {
  ...
  if (movie.title.length < 5 || movie.title.length > 255) {
    return new ValidateResult(
      false,
      'Invalid parameter title (Must be at least 5 and maximum 255 characters length).'
    );
  }
  ...
  if (isNaN(movie.numberInStock)) {
    return new ValidateResult(
      false,
      'Invalid parameter numberInStock (Not a number).'
    );
  }
  ...
  if (numberInStock < 0 || numberInStock > 255) {
    return new ValidateResult(
      false,
      'Invalid parameter numberInStock (Must be at least 0 and maximum 255).'
    );
  }
  ...
  if (isNaN(movie.dailyRentalRate)) {
    return new ValidateResult(
      false,
      'Invalid parameter dailyRentalRate (Not a number).'
    );
  }
  ...
  if (dailyRentalRate < 0 || dailyRentalRate > 255) {
    return new ValidateResult(
      false,
      'Invalid parameter dailyRentalRate (Must be at least 0 and maximum 255).'
    );
  }
  return new ValidateResult(true, null);
};
