...
router.post('/', async (req, res) => {
  ...
  if (!genre) {
    return res
      .status(400)
      .send(
        `Genre not found (Id: ${req.body.genreId.trim()}) on the database.`
      );
  }
...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    console.error(
      `Failed to update the movie (Id: ${req.params.id.trim()}).`,
      err
    );
  }
  ...
  if (!movie) {
    return res
      .status(400)
      .send(
        `Failed to update the movie (Id: ${req.params.id.trim()}) on the database.`
      );
  }
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    console.error(
      `Failed to delete the movie (Id: ${req.params.id.trim()}).`,
      err
    );
  }
  ...
  if (!movie) {
    return res
      .status(400)
      .send(
        `Failed to delete the movie (Id: ${req.params.id.trim()}) from the database.`
      );
  }
...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    console.error(
      `Failed to get the movie (Id: ${req.params.id.trim()}).`,
      err
    );
  }
  ...
  if (!movie) {
    return res
      .status(404)
      .send(
        `Failed to get the movie (Id: ${req.params.id.trim()}) from the database.`
      );
  }
