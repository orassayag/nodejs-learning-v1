...
router.put('/:id', async (req, res) => {
  ...
  try {
    ...
    console.error(
      `Failed to update the customer (Id: ${req.params.id.trim()}).`,
      err
    );
  }
  ...
  if (!customer) {
    return res
      .status(400)
      .send(
        `Failed to update the customer (Id: ${req.params.id.trim()}) on the database.`
      );
  }
...
router.delete('/:id', async (req, res) => {
  ...
  try {
    ...
    console.error(
      `Failed to delete the customer (Id: ${req.params.id.trim()}).`,
      err
    );
  }
  ...
  if (!customer) {
    return res
      .status(400)
      .send(
        `Failed to delete the customer (Id: ${req.params.id.trim()}) from the database.`
      );
  }
...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    console.error(
      `Failed to get the customer (Id: ${req.params.id.trim()}).`,
      err
    );
  }
  ...
  if (!customer) {
    return res
      .status(404)
      .send(
        `Failed to get the customer (Id: ${req.params.id.trim()}) from the database.`
      );
  }
