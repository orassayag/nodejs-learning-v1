...
router.get('/:id', (req, res) => {
  ...
  if (!genre) {
    return res
      .status(404)
      .send(`Genre with the Id of ${req.params.id} was not found.`);
  }
...
const validateGenre = (req) => {
  if (!req || !req.body.name) {
    return new ValidationResult(
      false,
      'No request exists or no genre name was provided.'
    );
  }
  ...
  if (req.body.name.length < 3) {
    return new ValidationResult(
      false,
      'Genre name was less than 3 characters length.'
    );
  }
...
const validateGenreId = (req) => {
  if (!req || !req.params.id) {
    return new ValidationResult(
      false,
      'No request exists or no genre Id was provided.'
    );
  }
